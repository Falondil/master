...
#1f78b4
#b2df8a
#33a02c for color template on the r, TIHZ(r) continuous barplot
...
# PARSEC concatenation must be done manually on a single track file
if fileselect == '1':
    track = []
...
# MIST does not require concatenation
    ...
    age = [float(track[i][0]) for i in range(12,n)]
    logL = [float(track[i][6]) for i in range(12,n)]
    logTe = [float(track[i][11]) for i in range(12,n)]
...
plt.ylabel('Luminosity [L_sun]')
plt.title('HR-esque diagram ('+solarmodel+')')
...
#     plt.title('Habitable zone track of '+bodies[nb]+' ('+solarmodel+')')
#     plt.xlabel('Instellation [S/S0]')
#     plt.ylabel('Effective temperature [K]')
